...
export async function sendPushNotification(userId, notification) {
  ...
  if (!app) return;
  try {
    const { rows } = await query(`SELECT raw_token FROM push_tokens WHERE user_id = $1 ORDER BY updated_at DESC LIMIT 1`, [userId]);
    if (!rows.length) return;
    const message = {
      token: rows[0].raw_token,
      notification: { title: notification.title, body: notification.body },
      data: notification.data ? Object.fromEntries(Object.entries(notification.data).map(([k, v]) => [k, String(v)])) : {},
    ...
    if (err.code === 'messaging/registration-token-not-registered') {
      await query(`DELETE FROM push_tokens WHERE user_id=$1 AND raw_token=(SELECT raw_token FROM push_tokens WHERE user_id=$1 ORDER BY updated_at DESC LIMIT 1)`, [userId]).catch(() => {});
    } else {
...
export const Notifications = {
  ...
  newMessage: (nick, preview = '새 메시지가 있습니다') => ({ title: nick, body: preview }),
