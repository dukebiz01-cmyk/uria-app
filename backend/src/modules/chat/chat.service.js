...
export async function getRoomMessages(userId, roomId, queryParams) {
  ...
  const messages = await chatRepo.listMessages(roomId, { cursor, limit: parseInt(limit) + 1 });
  ...
  return { room, messages: items, pagination: { has_more: hasMore, next_cursor: nextCursor } };
}
...
export async function sendMessage(userId, roomId, content) {
  ...
  sendPushNotification(otherUserId, { ...Notifications.newMessage(sender.nickname, preview), data: { room_id: roomId, message_id: message.id } }).catch(() => {});
  return message;
}
